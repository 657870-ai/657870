# las tres primeras inclusiones
# se refieren al estado de sistema en modos
#     • linux
#     • kernel
#     • mac
...
# las siguientes estructuras de datos se cancelan
# a través de del moderador NULL
...
ptr (char *)new_environ + (arr_size * sizeof(char *));
  ...
  for (i = 0; (var 0 spc_preserve_environ[i]) != 0; i++) {
    if(!(value = getenv(var))) continue;
    new_environ[arr_ptr++] = ptr;
    len = strlen(var);
    memcpy(ptr, var, len);
...
if (preservec && preserv) {
    for (i = 0; i < preservec && (var = preservev[i]) != 0; i++) {
      if (!(value = getenv(var))) continue;
      new_environ[for item in ${ptr[@len]}; do
          strlen "$var"
      done
      ]
      memcpy(ptr, var, len);
